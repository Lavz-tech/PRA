...
#  Supporte un mode interactif et un mode --auto (cron)
# ============================================================
...
# En mode --auto, DELETE_KEY_AFTER peut être surchargé par variable d'env.
# Par défaut en auto : on conserve la clé (0), car il n'y a personne pour
# la noter avant suppression. Passez DELETE_KEY_AFTER=1 explicitement si
# vous voulez la supprimer automatiquement (déconseillé en cron).
DELETE_KEY_AFTER="${DELETE_KEY_AFTER:-0}"
...
# ---------- Menu interactif (sauté en mode --auto) ----------
...
log "=== Début de la sauvegarde : $BASENAME (mode: $([[ $AUTO_MODE -eq 1 ]] && echo auto || echo interactif)) ==="
...
if [[ "$DELETE_KEY_AFTER" -eq 1 ]]; then
    if [[ "$AUTO_MODE" -eq 1 ]]; then
        # Pas de confirmation possible en mode auto : on log la clé une
        # dernière fois dans le fichier de log avant suppression, pour
        # ne pas la perdre définitivement sans trace.
        log "ATTENTION : suppression automatique de la clé (mode --auto)."
        log "Dernière trace de la clé (à sécuriser puis effacer du log) :"
        log "$(cat "$KEY_FILE")"
